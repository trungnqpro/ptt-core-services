 ...
 */
const ArtifactTypeSchema = require('./schema-mg')
 ...
 * @returns {Array<ArtifactType>} Mảng các tài khoản
 */
...
exports.fetch = async (skip = 0, limit = 20, filter = {}, sort = {}) => {
    ...
    return await ArtifactTypeSchema.find(filter).sort(sort).skip(skip).limit(limit).lean({ getters: true })
}
...
exports.create = async entity => {
    const result = await ArtifactTypeSchema.create(entity)
...
exports.getById = async id => {
    return await ArtifactTypeSchema.findOne({ _id: id }).lean()
}
...
exports.updateById = async (id, updatedFields) => {
    return await ArtifactTypeSchema.findByIdAndUpdate(id, updatedFields, {
        new: true,
    }).lean()
}
...
exports.deleteById = async id => {
    return await ArtifactTypeSchema.deleteOne({ _id: id })
}

exports.getTotalNumber = async (filter = {}) => {
    return await ArtifactTypeSchema.countDocuments(filter)
}
