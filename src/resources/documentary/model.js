 ...
 */
const DocumentarySchema = require('./schema-mg')
 ...
 * @returns {Array<Documentary>} Mảng các tài khoản
 */
...
exports.fetch = async (skip = 0, limit = 20, filter = {}, sort = {}) => {
    ...
    return await DocumentarySchema.find(filter).sort(sort).skip(skip).limit(limit).lean({ getters: true })
}
...
exports.create = async entity => {
    const result = await DocumentarySchema.create(entity)
...
exports.getById = async id => {
    return await DocumentarySchema.findOne({ _id: id }).lean()
}
...
exports.updateById = async (id, updatedFields) => {
    return await DocumentarySchema.findByIdAndUpdate(id, updatedFields, {
        new: true,
    }).lean()
}
...
exports.deleteById = async id => {
    return await DocumentarySchema.deleteOne({ _id: id })
}

exports.getTotalNumber = async (filter = {}) => {
    return await DocumentarySchema.countDocuments(filter)
}
