 ...
 */
const FileUploadSchema = require('./schema-mg')
 ...
 * @returns {Array<FileUpload>} Mảng các tài khoản
 */
...
exports.fetch = async (skip = 0, limit = 20, filter = {}, sort = {}) => {
    ...
    return await FileUploadSchema.find(filter).sort(sort).skip(skip).limit(limit).lean({ getters: true })
}
...
exports.create = async entity => {
    const result = await FileUploadSchema.create(entity)
...
exports.getById = async id => {
    return await FileUploadSchema.findOne({ _id: id }).lean()
}
...
exports.updateById = async (id, updatedFields) => {
    return await FileUploadSchema.findByIdAndUpdate(id, updatedFields, {
        new: true,
    }).lean()
}
...
exports.deleteById = async id => {
    return await FileUploadSchema.deleteOne({ _id: id })
}

exports.getTotalNumber = async (filter = {}) => {
    return await FileUploadSchema.countDocuments(filter)
}
