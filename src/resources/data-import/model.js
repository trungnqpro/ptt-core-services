 ...
 */
const DataImportSchema = require('./schema-mg')
 ...
 * @returns {Array<DataImport>} Mảng các tài khoản
 */
...
exports.fetch = async (skip = 0, limit = 20, filter = {}, sort = {}) => {
    ...
    return await DataImportSchema.find(filter).sort(sort).skip(skip).limit(limit).lean({ getters: true })
}
...
exports.create = async role => {
    return await DataImportSchema.create(role)
}
...
exports.getById = async id => {
    return await DataImportSchema.findOne({ _id: id }).lean()
}
...
exports.updateById = async (id, updatedFields) => {
    return await DataImportSchema.findByIdAndUpdate(id, updatedFields, {
        new: true,
    }).lean()
}
...
exports.deleteById = async id => {
    return await DataImportSchema.deleteOne({ _id: id })
}

exports.getTotalNumber = async (filter = {}) => {
    return await DataImportSchema.countDocuments(filter)
}
