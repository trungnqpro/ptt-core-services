...
const { PptRelsFactory } = require('./rels')
...
const { MediaFactory } = require('./media')
const { EmbeddingsFactory } = require('./embeddings')
const { ChartFactory } = require('./charts')
...
class PptFactory {
    ...
    addSlide(slideName, layoutName) {
        this.presentationFactory.addSlideRefIdToGlobalList(
            this.pptRelsFactory.addPresentationToSlideRelationship(slideName),
        )
        return this.slideFactory.addSlide(slideName, layoutName)
    }

    removeSlide(slideName) {
        this.presentationFactory.removeSlideRefIdFromGlobalList(
            this.pptRelsFactory.removePresentationToSlideRelationship(slideName),
        )
    ...
    addImage(slide, image) {
        ...
        if (image.sourceType === 'file') {
            ...
            if (source && /image\/(\w+);/.exec(source) && /image\/(\w+);/.exec(source).length > 0)
                imageExt = /image\/(\w+);/.exec(source)[1]
            ...
            throw new Error(
                'Invalid "sourceType" specified in PptFactory.addImage(). Possible values: "base64," "file," or "binary."',
            )
        }

        const rId = this.slideFactory.addImageToSlideRelationship(slide, `../media/${mediaName}`) // TODO: don't generate target string here, do it in the function

        PptFactoryHelper.handleHyperlinkOptions(
            this,
            PptFactoryHelper.HyperlinkType.IMAGE,
            slide,
            image.options,
        )
        this.mediaFactory.addMedia(
            mediaName,
            source,
            image.sourceType === 'url' ? 'binary' : image.sourceType,
        ) // the image binary must be added to the "/media" folder
        image.autoSize()
    ...
    addText(slide, textBox) {
        PptFactoryHelper.handleHyperlinkOptions(
            this,
            PptFactoryHelper.HyperlinkType.TEXT,
            slide,
            textBox.options,
        )
    ...
    async addChart(slide, chart) {
        ...
        const workbookJSZip = await ExcelHelper.createWorkbook(chart.chartData)
        const workbookContentZipBinary = await workbookJSZip.generateAsync({ type: 'arraybuffer' })
        const worksheetName = `Microsoft_Excel_Sheet${
            this.parentFactory.getWorksheetCount() + 1
        }.xlsx`
