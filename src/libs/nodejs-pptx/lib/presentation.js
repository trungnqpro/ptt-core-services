...
const { PowerPointFactory } = require('./factories')
const { PptxUnitHelper } = require('./helpers/unit-helper')

const POWERPOINT_LAYOUTS = require('./layout-types').Layouts
...
class Presentation {
    ...
    layout(layout) {
        ...
        if (typeof layout === 'object' && layout.width && layout.height) {
            ...
            console.warn(
                `Invalid layout in Presentation.layout(). Using type '${POWERPOINT_LAYOUTS['LAYOUT_4x3'].type}' as the default.`,
            )
    ...
    async addSlide(config, layoutName) {
        ...
        if (arguments.length === 0)
            return this.powerPointFactory.addSlide(layoutName || this.defaultSlideLayout)
        ...
        if (arguments.length === 1 && typeof config === 'string')
            return this.powerPointFactory.addSlide(config || this.defaultSlideLayout)
    ...
    getSlide(slideNameOrNumber) {
        if (typeof slideNameOrNumber === 'number' && Number.isInteger(slideNameOrNumber)) {
            ...
            typeof slideNameOrNumber === 'string' &&
            !slideNameOrNumber.startsWith('slide')
        ) {
        ...
        for (const key in content) {
            if (Object.prototype.hasOwnProperty.call(content, key)) {
                ...
                if (ext === '.xml' || ext === '.rels') {
                    ...
                    const xml = builder.buildObject(content[key])
    ...
    async save(destination) {
        ...
        if (typeof destination === 'string') {
            ...
            throw new Error(
                'Invalid destination value in Presentation.save() - can only be a file name or callback function.',
            )
        }
    }
}
