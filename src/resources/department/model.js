 ...
 */
const DepartmentSchema = require('./schema-mg')
 ...
 * @returns {Array<Department>} Mảng các tài khoản
 */
...
exports.fetch = async (skip = 0, limit = 20, filter = {}, sort = {}) => {
    ...
    return await DepartmentSchema.find(filter).sort(sort).skip(skip).limit(limit).lean({ getters: true })
}
...
exports.create = async entity => {
    return await DepartmentSchema.create(entity)
}
...
exports.getById = async id => {
    return await DepartmentSchema.findOne({ _id: id }).lean()
}
...
exports.updateById = async (id, updatedFields) => {
    return await DepartmentSchema.findByIdAndUpdate(id, updatedFields, {
        new: true,
    }).lean()
}
...
exports.getTotalNumber = async (filter = {}) => {
    return await DepartmentSchema.countDocuments(filter)
}
