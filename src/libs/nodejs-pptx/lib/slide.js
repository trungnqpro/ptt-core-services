...
class Slide {
    ...
    async addImage(config) {
        ...
        try {
            ...
            throw new Error(
                `Exception in Slide.addImage() when calling this.processConfig(). ${err.message}`,
            )
        }
        ...
        try {
            ...
            throw new Error(
                `Failed to add image to slide. Image source: ${imageSource}. Exception info: ${err.message}`,
            )
        }
    }
    ...
    moveTo(destinationSlideNum) {
        try {
            const thisSlideNum = Number(this.name.replace('slide', ''))

            this.powerPointFactory.moveSlide(thisSlideNum, destinationSlideNum)
        } catch (err) {
            throw new Error(
                `Failed to move slide to new position #: ${destinationSlideNum}. Exception info: ${err.message}`,
            )
        }
    }
    ...
    getShapeRawContent(index) {
        let c = 0
        const shapeNodes = this.content['p:sld']['p:cSld'][0]['p:spTree'][0]['p:sp']

        for (let i = 0; i < shapeNodes.length; i++) {
            const currentShape = shapeNodes[i]

            if (
                currentShape['p:nvSpPr'] &&
                currentShape['p:nvSpPr'][0]['p:cNvSpPr'] &&
                currentShape['p:nvSpPr'][0]['p:cNvSpPr'][0]['$']
            ) {
                const txBoxNode = currentShape['p:nvSpPr'][0]['p:cNvSpPr'][0]['$'].txBox
