 ...
 */
const ConfigBackupSchema = require('./schema-mg')
 ...
 * @returns {Array<SystemBackup>} Mảng các tài khoản
 */
...
exports.fetch = async (skip = 0, limit = 20, filter = {}, sort = {}) => {
    ...
    return await ConfigBackupSchema.find(filter).sort(sort).skip(skip).limit(limit).lean({ getters: true })
}
...
exports.create = async entity => {
    const result = await ConfigBackupSchema.create(entity)
...
exports.getById = async id => {
    return await ConfigBackupSchema.findOne({ _id: id }).lean()
}
...
exports.updateById = async (id, updatedFields) => {
    return await ConfigBackupSchema.findByIdAndUpdate(id, updatedFields, {
        new: true,
    }).lean()
}
...
exports.deleteById = async id => {
    return await ConfigBackupSchema.deleteOne({ _id: id })
}

exports.getTotalNumber = async (filter = {}) => {
    return await ConfigBackupSchema.countDocuments(filter)
}
