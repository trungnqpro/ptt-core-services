const { PptFactoryHelper } = require('../../helpers/ppt-factory-helper')
...
class SlideFactory {
    ...
    addSlide(slideName, layoutName) {
        ...
        const layoutKey = Object.keys(this.content).find(
            key =>
                /ppt\/slideLayouts\/slideLayout.*\.xml/.test(key) &&
                this.content[key]['p:sldLayout']['p:cSld'][0].$.name === layoutName,
        )
        const baseSlideContent = this.content[layoutKey]['p:sldLayout']
    ...
    swapSlide(slideNum1, slideNum2) {
        const slideKey1 = `ppt/slides/slide${slideNum1}.xml`
        const slideKey2 = `ppt/slides/slide${slideNum2}.xml`
        const slideRelsKey1 = `ppt/slides/_rels/slide${slideNum1}.xml.rels` // you need to swap rels in case slide layouts are used
        const slideRelsKey2 = `ppt/slides/_rels/slide${slideNum2}.xml.rels`

        ;[this.content[slideKey1], this.content[slideKey2]] = [
            this.content[slideKey2],
            this.content[slideKey1],
        ]
        ;[this.content[slideRelsKey1], this.content[slideRelsKey2]] = [
            this.content[slideRelsKey2],
            this.content[slideRelsKey1],
        ]
    }

    addImageToSlideRelationship(slide, target) {
        const relsKey = `ppt/slides/_rels/${slide.name}.xml.rels`
        const rId = `rId${this.content[relsKey]['Relationships']['Relationship'].length + 1}`
    ...
    addImage(slide, image, imageObjectName, rId) {
        ...
        this.content[slideKey]['p:sld']['p:cSld'][0]['p:spTree'][0]['p:nvGrpSpPr'][0][
            'p:cNvPr'
        ].forEach(function (element) {
            objectCount++
        })

        const spTreeRoot = this.content[slideKey]['p:sld']['p:cSld'][0]['p:spTree'][0]

        // won't have sp nodes on a blank slide
        if (spTreeRoot['p:sp']) {
            this.content[slideKey]['p:sld']['p:cSld'][0]['p:spTree'][0]['p:sp'].forEach(function (
                element,
            ) {
        ...
        const picObjectId = slide.getNextObjectId()
                    ...
                    'p:cNvPr': [
                        {
                            $: {
                                id: objectCount + 1,
                                name: `${imageObjectName} ${objectCount + 1}`,
                                descr: imageObjectName,
                            },
                        },
                    ],
        ...
        if (typeof image.options.url === 'string' && image.options.url.length > 0) {
            newImageBlock['p:nvPicPr'][0]['p:cNvPr'][0]['a:hlinkClick'] = [
                { $: { 'r:id': image.options.rIdForHyperlink } },
            ]
            if (image.options.url[0] === '#')
                newImageBlock['p:nvPicPr'][0]['p:cNvPr'][0]['a:hlinkClick'][0]['$'].action =
    ...
    addText(slide, textBox) {
        const slideKey = `ppt/slides/${slide.name}.xml`
        const objectId = slide.getNextObjectId()
        ...
        const newTextBlock = PptFactoryHelper.createBaseShapeBlock(
            objectId,
            'Text',
            textBox.x(),
            textBox.y(),
        ...
        if (options.backgroundColor) {
            newTextBlock['p:spPr'][0]['a:solidFill'] = [
                PptFactoryHelper.createColorBlock(options.backgroundColor),
            ]
        } else {
        ...
        PptFactoryHelper.setTextBodyProperties(
            newTextBlock['p:txBody'][0]['a:bodyPr'][0],
            textBox,
            options,
        )
        ...
        if (typeof options.url === 'string' && options.url.length > 0) {
            newTextBlock['p:txBody'][0]['a:p'][0]['a:r'][0]['a:rPr'][0]['a:hlinkClick'] = [
                { $: { 'r:id': options.rIdForHyperlink } },
            ]

            if (options.url[0] === '#')
                newTextBlock['p:txBody'][0]['a:p'][0]['a:r'][0]['a:rPr'][0]['a:hlinkClick'][0][
                    '$'
    ...
    addShape(slide, shape) {
        ...
        const shapeColor = options.color || '00AA00'
        ...
        const newShapeBlock = PptFactoryHelper.createBaseShapeBlock(
            objectId,
            'Shape',
            shape.x(),
            shape.y(),
        ...
        PptFactoryHelper.setTextBodyProperties(
            newShapeBlock['p:txBody'][0]['a:bodyPr'][0],
            shape,
            options,
        )
        ...
        if (typeof options.url === 'string' && options.url.length > 0) {
            newShapeBlock['p:nvSpPr'][0]['p:cNvPr'][0]['a:hlinkClick'] = [
                { $: { 'r:id': options.rIdForHyperlink } },
            ]
            if (options.url[0] === '#')
                newShapeBlock['p:nvSpPr'][0]['p:cNvPr'][0]['a:hlinkClick'][0]['$'].action =
                    'ppaction://hlinksldjump'
        }
    ...
    addChart(slide, chart) {
        const slideKey = `ppt/slides/${slide.name}.xml`
        const chartKey = `ppt/charts/${chart.name}.xml`

        const newGraphicFrameBlock = PptFactoryHelper.createBaseChartFrameBlock(
            chart.x(),
            chart.y(),
            chart.cx(),
            chart.cy(),
        ) // goes onto the slide
        const newChartSpaceBlock = PptFactoryHelper.createBaseChartSpaceBlock() // goes into the chart XML
        const seriesDataBlock = PptFactoryHelper.createSeriesDataBlock(chart.chartData)

        newChartSpaceBlock['c:chartSpace']['c:chart'][0]['c:plotArea'][0]['c:barChart'][0][
            'c:ser'
        ] = seriesDataBlock['c:ser']

        this.content[chartKey] = newChartSpaceBlock
        this.content[slideKey]['p:sld']['p:cSld'][0]['p:spTree'][0]['p:graphicFrame'] =
            newGraphicFrameBlock['p:graphicFrame']
    ...
    setBackgroundColor(slide, color) {
        const slideKey = `ppt/slides/${slide.name}.xml`
        const slideContent = this.content[slideKey]['p:sld']['p:cSld'][0]

        if (slideContent['p:bg'] !== undefined) {
            if (slideContent['p:bg'][0]['p:bgPr'] === undefined) {
                slideContent['p:bg'][0]['p:bgPr'] = [{}]
            } else {
                for (const key in slideContent['p:bg'][0]['p:bgPr'][0]) {
                    if (
                        Object.prototype.hasOwnProperty.call(
                            slideContent['p:bg'][0]['p:bgPr'][0],
                            key,
                        )
                    ) {
            ...
            slideContent['p:bg'][0]['p:bgPr'][0]['a:solidFill'] = [
                PptFactoryHelper.createColorBlock(color),
            ]
            ...
            const existingNodes = PptxContentHelper.extractNodes(slideContent) // this also deletes the nodes from slideContent
