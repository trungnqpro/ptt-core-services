...
exports.addSession = async (key, socketId) => {
    return redis.sadd(Redis.genKey('sockets/guests:connections', `id:${key}`), socketId)
}

exports.removeSession = async (key, socketId) => {
    redis.srem(Redis.genKey('sockets/guests:connections', `id:${key}`), socketId).catch(error => {
