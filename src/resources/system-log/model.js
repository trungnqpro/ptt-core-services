 ...
 */
const ConfigLogSchema = require('./schema-mg')
 ...
 * @returns {Array<ConfigLog>} Mảng các tài khoản
 */
...
exports.fetch = async (skip = 0, limit = 20, filter = {}, sort = {}) => {
    ...
    return await ConfigLogSchema.find(filter).sort(sort).skip(skip).limit(limit).lean({ getters: true })
}
...
exports.create = async entity => {
    const result = await ConfigLogSchema.create(entity)
...
exports.getById = async id => {
    return await ConfigLogSchema.findOne({ _id: id }).lean()
}
...
exports.updateById = async (id, updatedFields) => {
    return await ConfigLogSchema.findByIdAndUpdate(id, updatedFields, {
        new: true,
    }).lean()
}
...
exports.deleteById = async id => {
    return await ConfigLogSchema.deleteOne({ _id: id })
}

exports.getTotalNumber = async (filter = {}) => {
    return await ConfigLogSchema.countDocuments(filter)
}
