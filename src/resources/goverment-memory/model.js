 ...
 */
const GovermentPostSchema = require('./schema-mg')
 ...
 * @returns {Array<GovermentMemory>} Mảng các tài khoản
 */
...
exports.fetch = async (skip = 0, limit = 20, filter = {}, sort = {}) => {
    ...
    return await GovermentPostSchema.find(filter).sort(sort).skip(skip).limit(limit).lean({ getters: true })
}
...
exports.create = async entity => {
    const result = await GovermentPostSchema.create(entity)
...
exports.getById = async id => {
    return await GovermentPostSchema.findOne({ _id: id }).lean()
}
...
exports.updateById = async (id, updatedFields) => {
    return await GovermentPostSchema.findByIdAndUpdate(id, updatedFields, {
        new: true,
    }).lean()
}
...
exports.deleteById = async id => {
    return await GovermentPostSchema.deleteOne({ _id: id })
}

exports.getTotalNumber = async (filter = {}) => {
    return await GovermentPostSchema.countDocuments(filter)
}
