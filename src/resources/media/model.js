 ...
 */
const MediaSchema = require('./schema-mg')
 ...
 * @returns {Array<Media>} Mảng các tài khoản
 */
...
exports.fetch = async (skip = 0, limit = 20, filter = {}, sort = {}) => {
    ...
    return await MediaSchema.find(filter).sort(sort).skip(skip).limit(limit).lean({ getters: true })
}
...
exports.create = async role => {
    return await MediaSchema.create(role)
}
...
exports.getById = async id => {
    return await MediaSchema.findOne({ _id: id }).lean()
}
...
exports.updateById = async (id, updatedFields) => {
    return await MediaSchema.findByIdAndUpdate(id, updatedFields, {
        new: true,
    }).lean()
}
...
exports.deleteById = async id => {
    return await MediaSchema.deleteOne({ _id: id })
}

exports.getTotalNumber = async (filter = {}) => {
    return await MediaSchema.countDocuments(filter)
}
