 ...
 */
const PositionSchema = require('./schema-mg')
 ...
 * @returns {Array<Position>} Mảng các tài khoản
 */
...
exports.fetch = async (skip = 0, limit = 20, filter = {}, sort = {}) => {
    ...
    return await PositionSchema.find(filter).sort(sort).skip(skip).limit(limit).lean({ getters: true })
}
...
exports.create = async entity => {
    const result = await PositionSchema.create(entity)
...
exports.getTotalNumber = async (filter = {}) => {
    return await PositionSchema.countDocuments(filter)
}
