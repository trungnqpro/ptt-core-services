 ...
 */
const AlbumSchema = require('./schema-mg')
 ...
 * @returns {Array<Album>} Mảng các tài khoản
 */
...
exports.fetch = async (skip = 0, limit = 20, filter = {}, sort = {}) => {
    ...
    return await AlbumSchema.find(filter).sort(sort).skip(skip).limit(limit).lean({ getters: true })
}
...
exports.create = async role => {
    return await AlbumSchema.create(role)
}
...
exports.getById = async id => {
    return await AlbumSchema.findOne({ _id: id }).lean()
}
...
exports.updateById = async (id, updatedFields) => {
    return await AlbumSchema.findByIdAndUpdate(id, updatedFields, {
        new: true,
    }).lean()
}
...
exports.deleteById = async id => {
    return await AlbumSchema.deleteOne({ _id: id })
}

exports.getTotalNumber = async (filter = {}) => {
    return await AlbumSchema.countDocuments(filter)
}
