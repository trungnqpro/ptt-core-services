...
class PptRelsFactory {
    ...
    build() {
        xml2js.parseString(
            fs.readFileSync(`${__dirname}/../../fragments/ppt/_rels/presentation.xml.rels`),
            (err, js) => {
    ...
    removePresentationToSlideRelationship(slideName) {
        ...
        const target = `slides/${slideName}.xml`

        this.content['ppt/_rels/presentation.xml.rels']['Relationships']['Relationship'].forEach(
        ...
        if (relationshipIndex !== -1) {
            this.content['ppt/_rels/presentation.xml.rels']['Relationships']['Relationship'].splice(
                relationshipIndex,
                1,
            )
        }
